...
namespace PhanMem
{
    public partial class BcTongHopMay : Form
    {
        ...
        private void btExport_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    sfdXuatExcel.FileName = "May-TongHop-" + DateTimeSQLite(dtpTuNgay.Value) + "-" + DateTimeSQLite(dtpDenNgay.Value) + ".xlsx";
                    ...
                    if (sfdXuatExcel.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        string source = DA.SourceFile() + @"Data\May-TongHop.xlsx";
                        string des = @"D:\May-TongHop.xlsx";
                        if (File.Exists(des))
                            File.Delete(des);
                        ...
                        {
                            DataTable dtct = DA.May_TongHopSP();
        ...
        private void btXNT_Click(object sender, EventArgs e)
        {
            ...
            {
                DA.Edit_ThamSo(GetParam());
                sfdXuatExcel.FileName = "May-XNT-" + DateTimeSQLite(dtpDenNgay.Value) + ".xlsx";
                sfdXuatExcel.Filter = "Excel File|*.xlsx";
                if (sfdXuatExcel.ShowDialog() == DialogResult.OK)
                {
                    if (File.Exists(sfdXuatExcel.FileName))
                        File.Delete(sfdXuatExcel.FileName);
                    string source = DA.SourceFile() + @"Data\May-XNT.xlsx";
                    string des = @"D:\May-XNT.xlsx";
                    if (File.Exists(des))
                        File.Delete(des);
                    string newdes = sfdXuatExcel.FileName;
                    File.Copy(source, des);
                    ...
                    using (ExcelPackage package = new ExcelPackage(new FileInfo(des)))
                    {
                        ...
                        if (rownum > 0)
                        {
                            for (int n = 0; n < rownum; n++)
                            {
                                ...
                                worksheet.Cells[n + 6, 4].Value = Convert.ToInt32(dtct.Rows[n]["TongSPCat"].ToString());
                                worksheet.Cells[n + 6, 5].Value = Convert.ToInt32(dtct.Rows[n]["TongSPMay"].ToString());
                            }
                            worksheet.Cells[5, 4].Value = Convert.ToInt32(dtct.Compute("SUM(TongSPCat)", "").ToString());
                            worksheet.Cells[5, 5].Value = Convert.ToInt32(dtct.Compute("SUM(TongSPMay)", "").ToString());

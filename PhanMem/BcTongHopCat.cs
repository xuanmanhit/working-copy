...
namespace PhanMem
{
    public partial class BcTongHopCat : Form
    {
        ...
        private void btThem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    sfdXuatExcel.FileName = "Cat-TongHop-" + DateTimeSQLite(dtpTuNgay.Value) + "-" + DateTimeSQLite(dtpDenNgay.Value) + ".xlsx";
                    ...
                    if (sfdXuatExcel.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        string source = DA.SourceFile() + @"Data\Cat-TongHop.xlsx";
                        string des = @"D:\Cat-TongHop.xlsx";
                        if (File.Exists(des))
                            File.Delete(des);
                        ...
                        using (ExcelPackage package = new ExcelPackage(new FileInfo(des)))
                        {
                            ...
                            for (int i = 0; i < rowCount; i++)
                            {
                                ...
                                worksheet.Cells[i + 6, 2].Value = dt.Rows[i]["KhoVai"].ToString();
                                worksheet.Cells[i + 6, 3].Value = double.Parse(dt.Rows[i]["TongKgCat"].ToString());
                                worksheet.Cells[i + 6, 4].Value = Convert.ToInt32(dt.Rows[i]["TongOng"].ToString());
                                worksheet.Cells[i + 6, 5].Value = double.Parse(dt.Rows[i]["TongKgSP"].ToString());
                                worksheet.Cells[i + 6, 6].Value = Convert.ToInt32(dt.Rows[i]["TongSP"].ToString());
                            }
